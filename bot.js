...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['by Mihawk','Mihawk','ZR1 Team'];
    var i = -1;
    var j = 0;
...
if (message.content.startsWith(prefix + 'by Mihawk')) { /// This is The DMS Code Send The Help In DMS // Code By NotGucci
    let pages = [`**
        ***__General orders__***
**
...
msg.react('??')
.then(() => msg.react(':slight_smile:'))
.then(() =>msg.react(':upside_down:'))
...
var args = message.content.split(" ").slice(1);
    ...
    let muterole = message.guild.roles.find(`name`, "انجب");
...
client.on('message', async msg => { // eslint-disable-line
	...
	//by ,$ ™𝒁𝑹1 | MIHAWK 👻็็็็็็็็็็็็็็#2947 'CODES SERVER'
...
//by ,$ ™𝒁𝑹1 | MIHAWK 👻็็็็็็็็็็็็็็#2947 'CODES SERVER'
	if (command === `p`) {
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			try {//by ,$ ™𝒁𝑹1 | MIHAWK 👻็็็็็็็็็็็็็็#2947 'CODES SERVER'
